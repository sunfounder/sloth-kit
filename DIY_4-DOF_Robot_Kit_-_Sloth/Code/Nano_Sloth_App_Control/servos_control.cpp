...
void servo_move(uint8_t servo_num, int8_t angle) {
    switch (servo_num) {
        case 0:
            RU.slowmove (int8_t(pgm_read_byte(&array_cal[0])) + angle, speed);
            delay(delay_step);
            break;
        case 1:
            RL.slowmove (int8_t(pgm_read_byte(&array_cal[1])) + angle, speed);
            delay(delay_step);
            break;
        case 2:
            LU.slowmove (int8_t(pgm_read_byte(&array_cal[2])) + angle, speed);
            delay(delay_step);
            break;
        case 3:
            LL.slowmove (int8_t(pgm_read_byte(&array_cal[3])) + angle, speed);
            delay(delay_step);
            break;
        default:
            break;
...
void do_action(int8_t len, int32_t array_addr) {
    for(int i=0; i<len; i++) {
        RU.slowmove (int8_t(pgm_read_byte(&array_cal[0])) + int8_t(pgm_read_byte(array_addr+SERVOS_NUM*i+0)) , speed);    
        RL.slowmove (int8_t(pgm_read_byte(&array_cal[1])) + int8_t(pgm_read_byte(array_addr+SERVOS_NUM*i+1)) , speed);
        LU.slowmove (int8_t(pgm_read_byte(&array_cal[2])) + int8_t(pgm_read_byte(array_addr+SERVOS_NUM*i+2)) , speed);
        LL.slowmove (int8_t(pgm_read_byte(&array_cal[3])) + int8_t(pgm_read_byte(array_addr+SERVOS_NUM*i+3)) , speed);
        delay(delay_step);
    }
}
...
void turn_right() {do_action(6, &array_turn_right);}
void stand() {do_action(1, &array_stand);}
void moon_walk_left() {do_action(5, &array_moon_walk_left);}
void moon_walk_right() {do_action(5, &array_moon_walk_right);}
void hook() {do_action(1, &array_hook);}
void big_swing() {do_action(1, &array_big_swing);}
